...
namespace TKS_Thuc_Tap_V11_Data_Access.Controller.Cache
{
    ...
    {
        public static List<CDM_Loai_San_Pham> Arr_Data = new List<CDM_Loai_San_Pham>();

        private static Dictionary<string, CDM_Loai_San_Pham> Dic_Data_Code = new Dictionary<string, CDM_Loai_San_Pham>();
        private static Dictionary<string, CDM_Loai_San_Pham> Dic_Data_Ten_LSP = new Dictionary<string, CDM_Loai_San_Pham>();
        private static Dictionary<long, CDM_Loai_San_Pham> Dic_Data_ID = new Dictionary<long, CDM_Loai_San_Pham>();
        public static void Load_Cache_Loai_San_Pham()
        {
            Arr_Data.Clear();
            Dic_Data_ID.Clear();
            Dic_Data_Code.Clear();
            Dic_Data_Ten_LSP.Clear();
            CDM_Loai_San_Pham_Controller v_objCtrl = new();
            List<CDM_Loai_San_Pham> v_arrTemp_Data = v_objCtrl.FQ_536_LSP_sp_sel_List_For_Cache();
        ...
        public static void Add_Data(CDM_Loai_San_Pham p_objData)
        {
            ...
            if (Dic_Data_Code.ContainsKey(p_objData.Ma_LSP.ToLower()) == false)
                Dic_Data_Code.Add(p_objData.Ma_LSP.ToLower(), p_objData);

            if (Dic_Data_Ten_LSP.ContainsKey(p_objData.Ten_LSP.ToLower()) == false)
                Dic_Data_Ten_LSP.Add(p_objData.Ten_LSP.ToLower(), p_objData);
        }
        ...
        public static void Delete_Data(long p_iAuto_ID)
        {
            ...
            Dic_Data_Code.Remove(v_objData.Ma_LSP.ToLower());
            Dic_Data_Ten_LSP.Remove(v_objData.Ten_LSP.ToLower());
        }
        ...
        public static CDM_Loai_San_Pham Get_Data_By_Ten_Don_Vi_Tinh(string p_strTen_Don_Vi_Tinh)
        {
            if (Dic_Data_Ten_LSP.ContainsKey(p_strTen_Don_Vi_Tinh.ToLower()) == true)
                return Dic_Data_Ten_LSP[p_strTen_Don_Vi_Tinh.ToLower()];
